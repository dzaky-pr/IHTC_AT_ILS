...
public class PA_ILS {
    ...
    public List<IHTP_Solution.OutputPatient> searchFeasibleSolution(List<IHTP_Solution.OutputPatient> initialSolution,
        ...
        // PA-ILS uses ALL allocated time to find feasible solution
        // IHTP_Solution will manage remaining time for AT-ILS optimization
        System.out.println("PA-ILS time limit: " + (maxTimeMs / 1000) + " seconds (" + String.format("%.2f", maxTimeMs / 60000.0) + " minutes)");
        ...
        final int MAX_NO_PROGRESS = 150; // Reduced from 200 to recover faster
        ...
        while (bestTotalHardViolations > 0) {
            // CRITICAL: Check time limit BEFORE starting any operations
            long elapsedMs = System.currentTimeMillis() - startTime;
            if (elapsedMs > maxTimeMs) {
                System.out.println("PA-ILS: Time limit reached at iteration " + iteration);
                System.out.println("PA-ILS: Best violations achieved: " + bestTotalHardViolations);
                System.out.println("PA-ILS: Elapsed time: " + (elapsedMs / 1000) + " seconds");
                // Log timeout
            ...
            // Early exit if feasible is found - return immediately to save time for AT-ILS
            if (bestTotalHardViolations == 0) {
                System.out.println("PA-ILS: Feasible solution found at iteration " + iteration + "!");
                System.out.println("PA-ILS: Elapsed time: " + (elapsedMs / 1000) + " seconds");
                violationLogs.add(new ViolationLogEntry(iteration, System.currentTimeMillis() - startTimeMs,
                        0, "FEASIBLE_FOUND"));
                break;
            ...
            if (improvement) {
                ...
                // Reduced threshold from 500 to 300 for faster recovery
                if (noProgressCount > 300 && bestH5 == 0 && bestTotalHardViolations > 0) {
                ...
                // Reduced threshold from 10 to 5 for more aggressive recovery
                if (noProgressCount > MAX_NO_PROGRESS && bestH5 == 0 && bestTotalHardViolations > 5) {
                    currentSolution = deepCopy(bestSolution);
                    noProgressCount = 0;
                    decreasingValue = 0.3;
                    calculateProbability();
                    System.out.println("PA-ILS: Reverted to best solution due to no progress");
                }
            }
